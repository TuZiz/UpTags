...
| 商店购买 | 在 `/tags shop` 里消耗货币购买 | `shop.yml` / `tags.yml` | 用点券、金币或称号币购买，模式通常是 `BUY`。 |
| 物品兑换 | 提交指定材料领取 | `shop.yml` / `tags.yml` | 背包材料足够时才会扣除，材料不足不会扣钱。 |
...
`tags.yml` 内带 `shop:` 节点的普通称号会自动生成商店商品。它们不一定写在独立的 `shop.yml` 里，但玩家仍然可以在商店流程里领取或兑换。
...
| 文件 | 用途 | 常改内容 |
| --- | --- | --- |
| `src/main/resources/tags.yml` | 定义称号本体。 | 显示名、描述、稀有度、默认解锁、升级组、内嵌普通商店规则。 |
| `src/main/resources/shop.yml` | 定义高级商品。 | 商品模式、挑战条件、价格、提交物品、图标、权限。 |
