...
class CustomTitleService(
    ...
    private fun handleNameInput(draft: CustomTitleDraft, preset: CustomTitlePreset, input: String): ValidationResult {
        ...
        preset.allowedPattern?.let { pattern ->
            val regex = runCatching { Regex(pattern) }.getOrElse { ex ->
                logger.warning("Invalid custom title allowed-pattern in preset ${preset.id}: ${ex.message}")
                return ValidationResult(false, "custom-title-invalid-pattern-config")
            }
            if (!regex.matches(UnicodeText.allowedPatternText(visible))) {
                return ValidationResult(false, "custom-title-invalid-pattern")
            }
        }
    ...
    private fun finalizeDraft(
        ...
        drafts.remove(player.uniqueId)
        repository.saveAsyncStrict(data) { result ->
            runForPlayer(player) {
    ...
    private fun handleCompletedOrderSaved(
        ...
        if (context.currencyType == CurrencyType.TITLE_COIN) {
            ...
            repository.saveAsyncStrict(data) { saveResult ->
                if (saveResult !is SaveResult.Success) {
                    logger.warning(
                        "Failed to persist refunded custom title order; player=${player.uniqueId}; order=${context.orderId}; result=${describeSaveResult(saveResult)}",
                    )
                }
            }
    ...
    fun customOrderIds(uniqueId: UUID): List<String> {
        return repository.get(uniqueId).customTitleOrders.keys.toList()
    }

    fun customOrderSummaries(uniqueId: UUID): List<String> {
        return repository.get(uniqueId).customTitleOrders.values
            .sortedByDescending { it.updatedAt }
            .map { order ->
                val reason = order.failureReason?.takeIf { it.isNotBlank() }?.let { "; reason=$it" }.orEmpty()
                "${order.orderId}; status=${order.status.name}; currency=${order.currencyType.name}; amount=${Support.formatDouble(order.currencyAmount)}; title=${order.titleId}$reason"
            }
    }

    fun adminMarkCustomOrderRefunded(uniqueId: UUID, orderId: String, actor: String): Boolean {
        val data = repository.get(uniqueId)
        val order = data.customTitleOrders[orderId] ?: run {
            logger.warning("Admin $actor tried to mark missing custom title order refunded; player=$uniqueId; order=$orderId")
            return false
        }
        if (order.status != CustomTitleOrderStatus.REFUND_PENDING) {
            logger.warning("Admin $actor tried to mark custom title order refunded from invalid state; player=$uniqueId; order=$orderId; status=${order.status}")
            return false
        }
        order.status = CustomTitleOrderStatus.REFUNDED
        order.failureReason = "Manually marked refunded by $actor"
        order.updatedAt = System.currentTimeMillis()
        repository.saveAsyncStrict(data) { result ->
            if (result !is SaveResult.Success) {
                logger.warning("Failed to save manual refunded order; actor=$actor; player=$uniqueId; order=$orderId; result=${describeSaveResult(result)}")
            }
        }
        logger.warning("Admin $actor marked custom title order refunded; player=$uniqueId; order=$orderId")
        return true
    }

    fun adminCompleteCustomOrder(uniqueId: UUID, orderId: String, actor: String): Boolean {
        val data = repository.get(uniqueId)
        val order = data.customTitleOrders[orderId] ?: run {
            logger.warning("Admin $actor tried to complete missing custom title order; player=$uniqueId; order=$orderId")
            return false
        }
        if (!data.customTitles.containsKey(order.titleId)) {
            logger.warning("Admin $actor tried to complete custom title order without title data; player=$uniqueId; order=$orderId; title=${order.titleId}")
            return false
        }
        order.status = CustomTitleOrderStatus.COMPLETED
        order.failureReason = null
        ...
        repository.saveAsyncStrict(data) { result ->
            if (result !is SaveResult.Success) {
                logger.warning("Failed to save manual completed order; actor=$actor; player=$uniqueId; order=$orderId; result=${describeSaveResult(result)}")
            }
        }
        logger.warning("Admin $actor marked custom title order completed; player=$uniqueId; order=$orderId")
        return true
    }
